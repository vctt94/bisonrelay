...
func (s *server) TipUser(ctx context.Context, req *types.TipUserRequest, resp *types.TipUserResponse) error {
	// Set a timeout of one second from now
	ctxd, cancel := context.WithTimeout(ctx, time.Second)
	defer cancel() // Ensure the context is canceled to free resources

	// Call the payment service's TipUser method with the new context
	err := s.paymentService.TipUser(ctxd, req, resp)
	if err != nil {
		return err
	}
